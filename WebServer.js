...
const web=http.createServer((request, response) => {
  ...
  request.on('error', (err) => {
	...
	//empezaron los comentarios locos
	...
	if (query!=null) {
		...
		//response.setHeader('Content-Type', 'application/json');
		if (query=="readdir") {
			console.log("reading dir... "+parsed.pathname);
			fs.readdir("."+parsed.pathname, (err, files) => {
				if (err) {
					response.end(JSON.stringify(err));
					return;
				  }
				var filter=parsed.searchParams.get("ext");
				if (filter) {
					var newFiles = [];
					files.forEach((file)=>{
						//if (!file.endsWith(filter)) files.splice(files.indexOf(file), 1);
						if (file.endsWith(filter)) newFiles.push(file);
					});
					files=newFiles;
				}
		...
		if (query=="auth") {
			...
			var user=parsed.searchParams.get("user");
			var pass=parsed.searchParams.get("pass");
			var loginResult="register";
			
			var id=-1;
			
			//sessions.forEach(function(ses,sesid) {
			sessions.every(function(ses) {
				if (ses.user==user) {
					if (ses.pass==pass) {
						loginResult="ok";
						id=ses.id
			...
			//var admin=false;
			//if (pass=="1das651dasdas51d") admin=true;//auth level admin
			//sessions[AlphaNum(4)+id]={user: , pass: , auth: (admin? 100: 0)};
			if (loginResult=="register") {
				id=Date.now();
				sessions.push({"id":id,"user": (user?user:""), "pass": (pass?pass:"")});
				sessionSave=true;
			}
			
			
			response.end(JSON.stringify({result:loginResult,session:id},null,"\t"));
			return;
		}
		...
		if (query=="datosgobar") {
			/*
			var sendQuery="";
			if (url.includes("&")) sendQuery=url.replace("query=datosgobar&","");
			else sendQuery=url.replace("?query=datosgobar","");*/
			/*fetch("https://datos.gob.ar/api/3/action"+sendQuery).then(function (response) {
				...
				// There was an error
				//ckan format
				response.end('{"help":"Error de servidor","success":false}');
				console.warn('Something went wrong.', err);
			});*/
			var sendQuery=url.includes('&')?url.replace("query=datosgobar&",""):url.replace("?query=datosgobar","");
			var options = {
				method: 'GET',
			...
			console.log("descargando http://"+options.hostname+options.path);
			/*
			fs.readFile("http://"+options.hostname+options.path, function(err, data) {
			  if (err) {
				response.end('{"error":"'+err.message+'","help":"Error de servidor,"success":false}');
			  } else {
				response.end(data); // Send the file data to the browser.
			  }
			});
			*/
			...
			https.get("https://"+options.hostname+":443/"+options.path, (res) => {
				console.log(res.headers);
				var resData={
					error:res,
					help:"Error desconocido",
					success:false
				};
			  ...
			  let error;
			  /*if (statusCode == 301 && "Location" in res.headers) {
			  ...
			  if (error) {
				console.log("error: "+error.message);
				  resData.help="Error de datos.gob.ar";
			  ...
			  let rawData = '';
			  res.on('data', (chunki) => rawData += chunki);
			  res.on('end', () => {
				response.end(rawData);
			  });
			}).on('error', (e) => {
			  console.log(`Got error: ${e.message}`);
			  resData.help="Error de servidor";
		...
		/*
		var rcon=parsed.searchParams.get("rcon");
		var admin=false;
		if (rcon=="1das651dasdas51d") admin=true;//auth level admin*/
		
		var index=parseInt(query);
		var new_resource=parsed.searchParams.get("resource");
		var parsed_resource={};
		
		if (method=="POST"||method=="PATCH") {
			if (new_resource) parsed_resource=JSON.parse(decodeURI(new_resource));
			else {
				res.error="null resource";
				response.end(JSON.stringify(res));
				return;
			}
		}
		...
		/*
			res.error="index is NaN";
			?query=1
			?query=id
			?query=20&resource={key:value}
			?query=msg&resource={key:value}
			?query=name&where=jhon&resource={key:value}
		*/
		
		if (isNaN(index)) {
			//buscar query en db.resources
			console.log("buscar "+query);
			res.results=[];
			var selected=[];
			//query = row
			//where = column
			var where=parsed.searchParams.get("where");
			
			var offset=parseInt(parsed.searchParams.get("offset"));
			if (offset<0||offset>db.resources.length-1||isNaN(offset)) offset=0;
			
			var max=parseInt(parsed.searchParams.get("max"));
			if (isNaN(max)) max=db.resources.length;
			
			for (var i=offset;i<db.resources.length;i++) {
				if (i>=max) break;
				if (!(query in db.resources[i])) continue;
				if (where==null) {
					selected.push(i);
					continue;
				}
				if (db.resources[query].includes(where)) {
					selected.push(i);
				}
			}
			if (selected.length>0) res.success=true;
			//var desc=parsed.searchParams.get("desc");
			if (parsed.searchParams.get("desc")) selected.reverse();
			
			if (method=="DELETE") {
				for (i=selected.length-1;i>=0;i--) {
					db.resources.splice(selected[i],1);
				}
				res.results=selected;
				dbSave=true;
			} else if (method=="GET") {
				for (i=0;i<selected.length;i++) {
					res.results.push(db.resources[selected[i]]);
				}
			} else if (method=="PATCH") {
				for (i=0;i<selected.length;i++) {
					res.results.push(selected[i]);
					db.resources[selected[i]]=Object.assign(db.resources[selected[i]],parsed_resource);
				}
				dbSave=true;
			} else if (method=="POST") {
				db.resources.push(parsed_resource);
				res.success=true;
				dbSave=true;
			}
		...
		if (index<0||index>db.resources.length-1) {
			res.error="index ("+index+") out of bounds, resources.length : "+db.resources.length;
		} else {
			//operación
			...
			if (method=="DELETE") {
				...
				res.result=db.resources.length;
				dbSave=true;
			} else if (method=="GET") {
				res.result=db.resources[index];
			} else if (method=="PATCH") {
				db.resources[index]=Object.assign(db.resources[index],parsed_resource);
				res.result=db.resources[index];
				dbSave=true;
			} else if (method=="POST") {
				db.resources[index]=parsed_resource;
				res.result=db.resources.length;
				dbSave=true;
			}
		}
