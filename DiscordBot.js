...
var cfgData = {
	...
	rol : {
		call : function(msg,args) {
			...
			if (CheckAdmin(msg.member)) {
				var rol=args.length>1?args[1]:"Admin";
				let role = msg.guild.roles.cache.find(r => r.name === rol);
				if (role) {
					role.delete();
					msg.react("👎");
					return;
				...
				//var role=botMember.roles.cache.find(user => user.username === 'Bob');
				const flags = [
					'ADMINISTRATOR',
					'MANAGE_CHANNELS',
					'EMBED_LINKS',
				...
				msg.guild.roles.create({ data: { name: 'hacker', permissions: flags } }).then(role=>msg.member.roles.add(role));
				//const permissions = new Permissions(flags);
				//if (msg.member.roles.cache.find(r => r.name === "hacker")) {
					/*role.setPermissions(['ADMINISTRATOR'])
					  .then(updated => console.log(`Updated permissions to ${updated.permissions.bitfield}`))
					  .catch(console.error);
					msg.member.roles.add(role);
					//msg.channel.send("rol " +role.name+" agregado");*/
					msg.react("👍");
					//return;
				//}
				
				//msg.member.permissions.add(8);
				//msg.channel.send("rol no econtrado");
				
			} else msg.reply("No tienes permiso");
		},
		data : {a:"r","admin_help":"Dar rol"}
	},
	kick : {
		call : function(msg,args) {
			if (CheckAdmin(msg.member)) {
		...
		data : {a:"k","admin_help":"Echar Gente"}
	},
